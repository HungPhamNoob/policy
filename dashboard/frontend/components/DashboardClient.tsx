...
type DashboardBootstrap = {
  ...
  points?: { points?: PredictionPoint[] };
  latest?: { predictions?: PredictionPoint[] };
  hotspots?: { hotspots?: Hotspot[] };
  replayHealth?: Record<string, unknown>;
  riskByHour?: { data?: Array<Record<string, number>> };
  severity?: { distribution?: Array<Record<string, number>> };
  weather?: { histogram?: Record<string, Array<{ bin: string; count: number }>> };
};
...
type PointsResponse = { points: PredictionPoint[] };
type LatestResponse = { predictions: PredictionPoint[] };
type HotspotsResponse = { hotspots: Hotspot[] };
type RiskByHourResponse = { data: Array<Record<string, number>> };
type SeverityResponse = { distribution: Array<Record<string, number>> };
type WeatherResponse = { histogram: Record<string, Array<{ bin: string; count: number }>> };

export function DashboardClient({ bootstrap }: { bootstrap: DashboardBootstrap }) {
  ...
  const summaryQuery = useQuery({
    ...
    initialData: mode === "replay" ? bootstrap.overview : undefined,
  ...
  const pointsQuery = useQuery<PointsResponse>({
    ...
    queryFn: () =>
      api.mapPoints({ limit: mapPointLimit, min_risk: minRisk, mode }) as Promise<PointsResponse>,
    initialData:
      mode === "replay" && minRisk === 0 && mapPointLimit === DASHBOARD_MAP_POINT_LIMITS.replay
        ? { points: bootstrap.points?.points || [] }
        : undefined,
    refetchInterval: 15_000,
    staleTime: 15_000,
    placeholderData: (previousData) => previousData
  ...
  const latestQuery = useQuery<LatestResponse>({
    ...
    queryFn: () => api.latest(DASHBOARD_LATEST_PREDICTIONS_LIMIT, mode) as Promise<LatestResponse>,
    initialData: mode === "replay" ? { predictions: bootstrap.latest?.predictions || [] } : undefined,
    refetchInterval: 15_000,
    staleTime: 15_000,
    placeholderData: (previousData) => previousData
  });
  const hotspotsQuery = useQuery<HotspotsResponse>({
    queryKey: ["hotspots", mode],
    queryFn: () => api.hotspots({ limit: 10, min_events: 1, mode }) as Promise<HotspotsResponse>,
    initialData: mode === "replay" ? { hotspots: bootstrap.hotspots?.hotspots || [] } : undefined,
    refetchInterval: 30_000,
    staleTime: 30_000,
    placeholderData: (previousData) => previousData
  });
  const replayHealthQuery = useQuery({
    queryKey: ["dashboard-replay-health"],
    queryFn: api.replayHealth,
    initialData: bootstrap.replayHealth,
    refetchInterval: 10_000,
    staleTime: 10_000,
    placeholderData: (previousData) => previousData
  });
  const riskByHourQuery = useQuery<RiskByHourResponse>({
    queryKey: ["risk-by-hour", mode],
    queryFn: () => api.riskByHour(mode) as Promise<RiskByHourResponse>,
    initialData: mode === "replay" ? { data: bootstrap.riskByHour?.data || [] } : undefined,
  ...
  const severityQuery = useQuery<SeverityResponse>({
    ...
    queryFn: () => api.severityDistribution(mode) as Promise<SeverityResponse>,
    initialData: mode === "replay" ? { distribution: bootstrap.severity?.distribution || [] } : undefined,
    refetchInterval: 600_000,
    staleTime: 600_000,
    placeholderData: (previousData) => previousData
  });
  const weatherQuery = useQuery<WeatherResponse>({
    ...
    queryFn: () => api.weatherHistogram(mode) as Promise<WeatherResponse>,
    initialData: mode === "replay" ? { histogram: bootstrap.weather?.histogram || {} } : undefined,
    refetchInterval: 600_000,
    staleTime: 600_000,
    placeholderData: (previousData) => previousData
  });
  ...
  const replayHealth = replayHealthQuery.data as Record<string, any> | undefined;
  ...
  const displayedTotalEvents = mode === "replay"
    ? replayProgressEvents
    : Number(summary.total_events ?? 0);
  const displayLatestEventTime = mode === "replay"
    ? String(
        replayPredictionSource?.latest_created_at ||
        replayPredictionSource?.latest_event_time ||
  ...
  const totalEventsDetail =
    mode === "replay" && displayedTotalEvents > 0
      ? `processed replay rows: ${displayedTotalEvents.toLocaleString()} | stored rows: ${Number(replayPredictionSource?.stored_row_count || summary.total_events || displayedTotalEvents).toLocaleString()}`
      : undefined;
  ...
  return (
    <div className="page-stack">
      ...
      <section className="grid kpi-grid">
        ...
        <KpiCard
          label="Latest event"
          value={displayLatestEventTime ? "Online" : "No data"}
          detail={
            displayLatestEventTime
              ? formatVietnamTimestamp(displayLatestEventTime, "Waiting for replay")
              : "Waiting for replay"
          }
        />
        <KpiCard
